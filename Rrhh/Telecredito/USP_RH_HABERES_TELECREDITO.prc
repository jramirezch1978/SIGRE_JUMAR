...
select bc.cod_banco, bc.cod_moneda
  into ls_cod_banco, ls_moneda
  from banco_cnta bc
...
if ls_cod_banco = '010' then
   -- Si el codigo de banco es del BCP
    ...
    ls_nro_cta_emp := ltrim(rtrim(replace(asi_cta_banco, '-', '')));
    ...
    -- NRO CUENTA
    lc_telecredito := lc_telecredito || substr(ltrim(rtrim(asi_cta_banco)),1,20);
    ...
    if asi_quincena = '0' then
       for rc_t in c_boleta loop
           ...
           ln_nro_item := ln_nro_item + 1;
           lc_telecredito := '002'|| 'L' || rpad(rc_t.dni, 12, ' ') || 'P' 
                          || lpad(rtrim(ltrim(substr(replace(rc_t.cuenta, '-', ''),1,20))),20, ' ')
                          || rpad(rtrim(ltrim(substr(REPLACE(rc_t.nombre, '?', 'N'),1,40))),40, ' ')
                          || lc_neto_trab
                          || rpad(' ',40, ' ')
                          || LPAD(rc_t.email || ' ' , 50, ' ')
                          || lpad(' ', 2, ' ')
