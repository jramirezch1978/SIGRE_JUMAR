create or replace procedure usp_rh_pago_variables (
  as_origen in char, ad_fec_proceso in date, as_nropla in char,
  as_tipo_trabaj in char, as_concepto in char ) is
...
-- Lectura de trabajadores para pagos variables
cursor c_personal is
  select v.cod_trabajador, v.imp_var, m.nro_cnta_ahorro
  from gan_desct_variable v, maestro m
  where v.cod_trabajador = m.cod_trabajador and m.flag_cal_plnlla = '1' and
        m.flag_estado = '1' and nvl(m.nro_cnta_ahorro,' ') <> ' ' and
        m.tipo_trabajador like as_tipo_trabaj and m.cod_origen = as_origen and
        v.concep = as_concepto and nvl(v.imp_var,0) <> 0 ;

begin

--  **************************************************
--  ***   REALIZA PAGOS DE MOVIMIENTOS VARIABLES   ***
--  **************************************************
...
select sum(nvl(v.imp_var,0)) into ln_neto_emp
  from gan_desct_variable v, maestro m
  where v.cod_trabajador = m.cod_trabajador and m.flag_cal_plnlla = '1' and
        m.flag_estado = '1' and nvl(m.nro_cnta_ahorro,' ') <> ' ' and
        m.tipo_trabajador like as_tipo_trabaj and m.cod_origen = as_origen and
...
for rc_t in c_personal loop
  ...
  ln_neto_trab := nvl(rc_t.imp_var,0) ;
