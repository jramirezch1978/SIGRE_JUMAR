create or replace procedure usp_rh_pago_rem_credito (
  as_origen in char, ad_fec_proceso in date, as_nropla in char,
  as_tipo_trabaj in char, as_cod_banco in char ) is
...
cursor c_trabajadores is
  select c.cod_trabajador, c.imp_soles, m.nro_cnta_ahorro
  ...
  where m.cod_trabajador = c.cod_trabajador and m.flag_cal_plnlla = '1' and
        ...
        m.cod_banco = as_cod_banco and c.concep = ls_concepto and nvl(c.imp_soles,0) <> 0
...
select sum(nvl(c.imp_soles,0)) into ln_neto_emp
  from calculo c, maestro m
  where c.cod_trabajador = m.cod_trabajador and m.flag_cal_plnlla = '1' and
        m.flag_estado = '1' and m.cod_origen = as_origen and
        m.tipo_trabajador like as_tipo_trabaj and m.cod_banco = as_cod_banco and
        m.nro_cnta_ahorro is not null and c.concep = ls_concepto and
        nvl(c.imp_soles,0) <> 0 ;
...
for rc_t in c_trabajadores loop
  ...
  ln_neto_trab := nvl(rc_t.imp_soles,0) ;
