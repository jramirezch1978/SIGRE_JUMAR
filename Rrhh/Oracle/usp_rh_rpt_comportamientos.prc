create or replace procedure usp_rh_rpt_comportamientos (
...
select nvl(rhc.desc_competencia,' ')
...
for rc_mae in c_maestro loop
  ...
  if ls_seccion <> ' ' then
    select nvl(s.desc_seccion,' ')
      ...
      from seccion s where s.cod_seccion = rc_mae.cod_seccion ;
  end if ;
...
end usp_rh_rpt_comportamientos ;
/
