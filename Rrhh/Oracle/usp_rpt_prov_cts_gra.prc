create or replace procedure usp_rpt_prov_cts_gra
...
for rc_mae in c_maestro loop
  ...
  ln_contador := 0 ; ls_descripcion := ' ' ;
  select count(*)
    into ln_contador from centros_costo cc
    where cc.cencos = ls_cencos ;
  if ln_contador > 0 then
    select nvl(cc.desc_cencos,' ')
      into ls_descripcion from centros_costo cc
      where cc.cencos = ls_cencos ;
  end if ;
  ...
  if ln_imp_prov_cts <> 0 or ln_imp_prov_gra <> 0 then
    insert into tt_rpt_prov_cts_gra (
      ...
      cencos, desc_cencos, imp_prov_cts,
      imp_prov_gra, fecha_proceso )
    values (
      ls_codigo, ls_nombres, ls_seccion,
      ls_cencos, ls_descripcion, ln_imp_prov_cts,
      ln_imp_prov_gra, ad_fec_proceso ) ;
  end if ;

end loop ;

end usp_rpt_prov_cts_gra ;
/
