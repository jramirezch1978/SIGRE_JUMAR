...
CURSOR c_maestro_hist is
  select distinct(m.cod_trabajador), m.dni, hc.cod_origen, 
         hc.tipo_trabajador, aos.cod_sede, ats.tipo_plla 
    from historico_calculo hc, maestro m, aux_origen_sede aos, aux_tipo_trab_sede ats  
   where hc.cod_trabajador = m.cod_trabajador and 
         hc.cod_origen = aos.cod_origen and
         hc.tipo_trabajador = ats.tipo_trabajador and  
    ...
    FOR rc_c in c_calculo(rc_m.cod_trabajador) LOOP 
        ...
        UPDATE tt_rpt_plla_calculada_cgsa tt 
           SET tt.c1001 = NVL(tt.c1001,0) + NVL(ln_1001,0), 
               ...
               tt.c1471 = NVL(tt.c1471,0) + NVL(ln_1471,0),
...
FOR rc_mh in c_maestro_hist loop
  ...
  FOR rc_h in c_historico(rc_mh.cod_trabajador) LOOP
        -- Inicializa variables
        ln_1001:=0;            ln_1002:=0;          ln_1003:=0;                     ln_1004:=0;
        ln_1005:=0;            ln_1006:=0;          ln_1007:=0;                     ln_1101:=0;
        ln_1102:=0;            ln_1103:=0;          ln_1104:=0;                     ln_1105:=0;
        ...
        ELSIF rc_h.concep = '7009' THEN
           ...
           IF ln_7009 > 0 THEN
              SELECT count(*) 
               ...
               WHERE tt.cod_trabajador=rc_mh.cod_trabajador and tt.c7009>0 ;
              IF ln_count>0 THEN
                 ln_7009 := 0 ;
              END IF ;
           END IF ;
        ...
        IF SQL%NOTFOUND THEN
            insert into tt_rpt_plla_calculada_cgsa(
              ...
              c7010, c7011, c7012, c7013) 
            values (
              rc_mh.cod_origen, rc_mh.tipo_trabajador, ad_fec_proceso_ini, ad_fec_proceso_fin, rc_mh.cod_trabajador, 
              ls_nave, rc_mh.cod_sede, rc_mh.tipo_plla, as_proceso, rc_h.mes, 
              ls_embarcacion, rc_mh.dni, ln_1001, ln_1002, ln_1003, 
              ln_1004, ln_1005, ln_1006, ln_1007, ln_1101, 
              ln_1102, ln_1103, ln_1104, ln_1105, ln_1201, 
              ln_1202, ln_1203, ln_1204, ln_1205, ln_1301, 
              ln_1302, ln_1401, ln_1402, ln_1403, ln_1405, 
