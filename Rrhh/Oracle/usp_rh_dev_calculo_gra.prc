create or replace procedure usp_rh_dev_calculo_gra (
...
--  **************************************************************
--  ***   CALCULO DE INTERESES DE GRATIFICACIONES DEVENGADAS   ***
--  **************************************************************

select c.gratific_deveng
  into lk_dev_gra
...
select count(*) into ln_verifica from sldo_deveng sd
  where sd.cod_trabajador = as_codtra and sd.fec_proceso = ad_fec_anterior and
        nvl(sd.sldo_gratif_dev,0) > 0 ;

if ln_verifica > 0 then

  for rc_gra in c_gratificacion loop
    if to_char(rc_gra.fec_calc_int,'mm/yyyy') <>
      to_char(ad_fec_proceso,'mm/yyyy') then
      ln_fac_pago_ant := nvl(rc_gra.fact_pago,0) ;
      ln_capital_ant  := nvl(rc_gra.nvo_capital,0) ;
      ln_interes_ant  := nvl(rc_gra.nvo_interes,0) ;
    end if ;
  end loop ;
  ...
  if ln_capital_ant > 0 or ln_interes_ant > 0 then
    ...
    select count(*) into ln_contador from maestro_remun_gratif_dev rgd
      where rgd.cod_trabajador = as_codtra and rgd.concep = ls_concepto and
            to_char(rgd.fec_calc_int,'mm/yyyy') = to_char(ad_fec_proceso,'mm/yyyy') ;
    ...
    if ln_contador = 0 then
      ...
      insert into maestro_remun_gratif_dev (
        ...
        fact_pago, fact_emplear, capital, imp_int_gen,
        imp_int_ant, adel_pago, nvo_capital, nvo_interes,
        int_pagado, mont_pagado, flag_replicacion )
      values (
        as_codtra, ad_fec_proceso, ls_concepto, '1', ad_fec_proceso,
        ln_fac_pago_act, ln_fact_emplear, ln_capital, ln_imp_int_gen,
        ln_imp_int_ant, ln_adel_pago, ln_nvo_capital, ln_nvo_interes,
        ln_int_pagado, ln_mont_pagado, '1' ) ;

    else

      for rc_gra in c_gratificacion loop

        if to_char(rc_gra.fec_calc_int,'mm/yyyy') = to_char(ad_fec_proceso,'mm/yyyy') then
