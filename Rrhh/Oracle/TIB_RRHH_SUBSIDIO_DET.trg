create or replace trigger TIB_RRHH_SUBSIDIO_DET
  before insert on rrhh_subsidio_det  
  for each row
declare
  -- local variables here
begin

    IF ( TRUNC(:new.fecha_fin_pla) - TRUNC(:new.fecha_ini_pla)) < 0 THEN
       RAISE_APPLICATION_ERROR( -20000, 'Rango de fechas a considerar es negativo' ) ;
       RETURN ;
    END IF ; 
