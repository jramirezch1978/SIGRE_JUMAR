...
SELECT m.cod_trabajador, m.dni, 
 ...
 WHERE hg.fecha_proceso=hd.fecha_proceso and 
       ...
       hd.cod_trabajador=m.cod_trabajador and
       hg.fecha_proceso=ad_fec_proceso 
...
DELETE FROM TT_HAYDUK_CTS; 

FOR c_cts IN c_data_cts LOOP
    ln_cts_soles := round((c_cts.rem_comp / 360) * c_cts.dias_asist,2) ;
    ...
    INSERT INTO TT_HAYDUK_CTS(COD_TRABAJADOR, DNI, NOMBRE, PERIODO, LUGAR_CTS, 
           ...
           TASA_INT_CTS, DIAS_ATRASADOS,TIPO_MONEDA) 
    VALUES(c_cts.cod_trabajador, c_cts.dni, SUBSTR(c_cts.nombre,1, 100), to_char(ad_fec_proceso,'yyyymm'), c_cts.lugar_cts, 
