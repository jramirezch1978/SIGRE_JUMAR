create or replace procedure usp_cal_rpt_fondo_retiro (
  as_tipo_trabajador in char, ad_fec_proceso in date ) is
...
ln_imp_07            number(13,2) ;
ln_imp_08            number(13,2) ;
ln_imp_09            number(13,2) ;
ln_imp_10            number(13,2) ;
ln_imp_11            number(13,2) ;
ln_imp_12            number(13,2) ;
ln_imp_13            number(13,2) ;
ln_imp_14            number(13,2) ;
ln_imp_15            number(13,2) ;
ln_imp_16            number(13,2) ;
ln_imp_17            number(13,2) ;
ln_imp_18            number(13,2) ;
ln_imp_19            number(13,2) ;
ln_imp_20            number(13,2) ;
ln_imp_21            number(13,2) ;
ln_imp_22            number(13,2) ;
ln_imp_23            number(13,2) ;
ln_imp_24            number(13,2) ;
...
cursor c_maestro is
  ...
  where m.flag_cal_plnlla = '1' and m.flag_estado = '1' and
        m.situa_trabaj = 'S' and m.tipo_trabajador = as_tipo_trabajador ;

--  Lectura de ganancias fijas por trabajador
cursor c_ganancias is
  select gdf.concep, gdf.imp_gan_desc
  from gan_desct_fijo gdf
  where gdf.cod_trabajador = ls_codigo and gdf.flag_estado = '1'
...
if as_tipo_trabajador = 'OBR' then
  ls_tipo_trabajador := '  O B R E R O S  ' ;
else
  ls_tipo_trabajador := 'E M P L E A D O S' ;
end if ;
...
for rc_mae in c_maestro loop
  ...
  if ln_imp_soles > 0 then
    insert into tt_rpt_fondo_retiro (
      fecha, tipo_trabajador, codigo, seccion,
      nombres, fec_ingreso, ano_descto, mes_descto,
      dia_descto, ano_increm, mes_imcrem, dia_increm,
      telano, telmes, teldia,
    ...
    values (
      ad_fec_proceso, ls_tipo_trabajador, ls_codigo, rc_mae.cod_seccion,
      ls_nombres, rc_mae.fec_ingreso, ln_ano_descto, ln_mes_descto,
      ln_dia_descto, ln_ano_increm, ln_mes_increm, ln_dia_increm,
      ln_telano, ln_telmes, ln_teldia,
