create or replace procedure usp_pla_gan_desct_fijo(
  as_cod_trabajador in maestro.cod_trabajador%type,
  as_concepto in gan_desct_fijo.concep%type
--pase de la descripcion de la formula in, out
--  as_desc_formula out formula.desc_formula%type 
...
--y la formula   
  For rc_gan_desct_fijo in c_gan_desct_fijo
    Loop
      ...
      ls_desc_formula:=rc_gan_desct_fijo.desc_formula;
  ...
  Insert into tt_pla_concepto(concep,formula,
                debe_haber,valor)
  values ('IMPB',ls_desc_formula,'0',0);
    
       
end usp_pla_gan_desct_fijo;
/
