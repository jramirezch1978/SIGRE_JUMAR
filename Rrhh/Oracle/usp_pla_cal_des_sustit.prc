create or replace procedure usp_pla_cal_des_sustit
...
ls_tipo_ina        char(3);
lk_gan_fij         constant char(3) := '099';
...
--  Obtiene concepto de descanso sustitutorio
select rpn.concep
  into ls_concep_des
  from rrhh_nivel rpn
  where rpn.cod_nivel = lk_gan_fij;

--  Determina dias de descanso sustitutorio
ln_diatra := 0 ;
...
--  Calcula descanso sustitutorio si tiene dias
If ln_diatra > 0 then
  ...
  For rc_c in c_concepto ( ls_concep_des ) Loop
