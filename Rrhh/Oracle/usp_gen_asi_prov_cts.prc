...
cursor c_maestro is
  ...
  where m.cod_trabajador = d.cod_trabajador and
       to_char(d.fec_proceso,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY')
...
cursor c_provision is
  select c.cod_trabajador, c.liquidacion
  from cts_decreto_urgencia c
  where c.cod_trabajador = ls_codigo and c.liquidacion <> 0 and
        to_char(c.fec_proceso,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY')
  order by c.fec_proceso, c.cod_trabajador ;
...
usp_cnt_borrar_pre_asiento( as_origen, lk_libro_cts, ld_fec_inicio, ad_fec_proceso ) ;
...
while c_maestro%found loop
  ...
  select count(*)
    ...
    where l.nro_libro = lk_libro_cts ;
  if ln_contador = 0 then
    ln_provisional := 1 ;
    insert into cntbl_libro ( nro_libro, desc_libro, num_provisional )
    values ( lk_libro_cts, substr(lk_desc_libro,1,40), ln_provisional ) ;
  else
  ...
  values (
    as_origen, lk_libro_cts, ln_provisional, 'S/.', ln_tipo_cambio,
  ...
  while rc_mae.cod_seccion = ls_seccion and c_maestro%found loop
    ...
    for rc_prov in c_provision loop
      ...
      ln_imp_soles := nvl(rc_prov.liquidacion,0) ;
      ...
      if ln_distribucion = 0 then
        ...
        values (
          as_origen, lk_libro_cts, ln_provisional, ln_item,
      ...
      if ln_distribucion = 1 then
        ...
        while c_distribucion%found loop
          ...
          if ls_flag_cp = '1' then
            ...
            values (
              as_origen, lk_libro_cts, ln_provisional, ln_item, ls_cnta_prsp ) ;
          end if ;
          select nvl(cta.flag_cencos,' '), nvl(cta.flag_codrel,' ')
            into ls_flag_cc, ls_flag_cr from cntbl_cnta cta
            where cta.cnta_ctbl = ls_cuenta_haber ;
  ...
  select sum(d.imp_movsol), sum(d.imp_movdol)
    ...
    where d.origen = as_origen and d.nro_libro = lk_libro_cts and
  ...
  update cntbl_pre_asiento
    ...
    where origen = as_origen and nro_libro = lk_libro_cts and
          nro_provisional = ln_provisional and fec_cntbl = ad_fec_proceso ;

end loop ;

end usp_gen_asi_prov_cts ;
/
