create or replace procedure usp_rpt_planilla_ingenio
  ( as_tipo_trabajador  in maestro.tipo_trabajador%type ) is
  
ld_fecha_proceso          rrhhparam.fec_proceso%type ;
ls_seccion                char(3) ;
...
For rc_mae in c_maestro Loop
  ...
  ln_contador := nvl(ln_contador,0) ;
  If ln_contador > 0 then
    Select cal.dias_trabaj, cal.horas_trabaj
  ...
  Select count(*)
    ...
    from calculo cal
    where cal.cod_trabajador = ls_codigo and
          substr(cal.concep,1,2) = '11' ;
  ln_contador := nvl(ln_contador,0) ;
  If ln_contador > 0 then
    Select sum(cal.horas_trabaj)
      into ln_var_03
      from calculo cal
      where cal.cod_trabajador = ls_codigo and
            substr(cal.concep,1,2) = '11' ;
  ...
  Values (
    ld_fecha_proceso, ls_seccion, ls_desc_seccion, ls_codigo,
