create or replace procedure usp_pla_dev_calculo_rem
   (as_codtra       in maestro.cod_trabajador%type ,
    ad_fec_proceso  in rrhhparam.fec_proceso%type
   ) is

lk_dev_gra         constant char(3) := '006' ;
ls_meses           char(2) ;
ls_years           char(4) ;
ln_contador        integer ;
...
-- Verifica si tiene saldo de remuneraciones para calulo de intereses
ln_contador := 0 ;
Select count(*)
  into ln_contador
  from sldo_deveng sd
...
For rc_rem in c_remuneracion Loop
  ...
  If to_char(rc_rem.fec_calc_int, 'MM')  <> ls_meses
  or to_char(rc_rem.fec_calc_int, 'YYYY') <> ls_years then
    ln_fac_pago_ant := rc_rem.fact_pago ;
    ln_capital_ant  := rc_rem.nvo_capital ;
    ln_interes_ant  := rc_rem.nvo_interes ;
  End if ;          
...
If ln_capital_ant > 0 or ln_interes_ant > 0 then
  ...
  If ln_contador = 0 then
    ...
    Insert into maestro_remun_gratif_dev
      ( cod_trabajador, fec_calc_int, concep,      flag_estado,
        ...
        mont_pagado )
     Values     
      ( as_codtra,       ad_fec_proceso, '1302',         '1',
  ...
  Else
    ...
    For rc_rem in c_remuneracion Loop
      ...
      If to_char(rc_rem.fec_calc_int, 'MM') = ls_meses
      and to_char(rc_rem.fec_calc_int, 'YYYY') = ls_years then
