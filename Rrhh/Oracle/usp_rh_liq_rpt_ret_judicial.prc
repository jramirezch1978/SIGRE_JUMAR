create or replace procedure usp_rh_liq_rpt_ret_judicial (
...
--  Lectura de movimiento de retencion judicial
cursor c_detalle is
...
--  ********************************************************************
--  ***   TEMPORAL PARA GENERAR LIQUIDACION POR RETENCION JUDICIAL   ***
--  ********************************************************************

delete from tt_liq_rpt_ret_judicial ;

select p.grp_dscto_remun into ls_grupo from rh_liqparam p
  where p.reckey = '1' ;
...
for rc_grp in c_grupos loop
  ...
  for rc_det in c_detalle loop
    ...
    insert into tt_liq_rpt_ret_judicial (
...
end usp_rh_liq_rpt_ret_judicial ;
/
