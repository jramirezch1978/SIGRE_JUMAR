create or replace procedure usp_pla_cal_vacaciones
...
ls_tipo_ina        char(3);
lk_gan_fij         constant char(3) := '082';
lk_gan_fij_25      constant char(3) := '083';
lk_gan_fij_30      constant char(3) := '084';
ls_concep_vac      concepto.concep%type;
ls_concep_vac25    concepto.concep%type;
ls_concep_vac30    concepto.concep%type;
...
If ln_diavac > 0 then
  ...
  For rc_c in c_concepto ( ls_concep_vac ) Loop
  ...
  If ls_bonif = '1' or ls_bonif = '2' then
    If ls_bonif = '2' then
       For rc_c in c_concepto ( ls_concep_vac25 ) Loop
       ...
       Insert into Calculo ( Cod_Trabajador, 
         ...
         Flag_e_Sctr_Ipss,     Flag_e_Sctr_Onp )
       Values ( as_codtra, 
         ls_concep_vac25, ad_fec_proceso,
         0            , 0             , ln_diavac, 
         ln_imp_soles , ln_imp_dolar  , rc_c.Flag_t_Snp, 
         rc_c.Flag_t_Quinta           , rc_c.Flag_t_Judicial, 
         rc_c.Flag_t_Afp              , rc_c.Flag_t_Bonif_30, 
       ...
       End Loop ;
    Else
       For rc_c in c_concepto ( ls_concep_vac30 ) Loop
         ln_factor := rc_c.fact_pago ;
         ln_imp_soles := ln_imp_soles * ln_factor ;
         ln_imp_dolar := ln_imp_soles / ln_tipcam ;
       Insert into Calculo ( Cod_Trabajador, 
         ...
         Flag_e_Sctr_Ipss,     Flag_e_Sctr_Onp )
       Values ( as_codtra, 
         ls_concep_vac30, ad_fec_proceso,
         0            , 0             , ln_diavac, 
         ln_imp_soles , ln_imp_dolar  , rc_c.Flag_t_Snp, 
         rc_c.Flag_t_Quinta           , rc_c.Flag_t_Judicial, 
         rc_c.Flag_t_Afp              , rc_c.Flag_t_Bonif_30, 
