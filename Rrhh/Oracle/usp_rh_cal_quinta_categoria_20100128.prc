...
begin
  ...
  --  categoría, según requerimiento de RRHH por intermedio de la Sra
  --  Patricia Cordoba Ballesteros, presentando el sustento legal con 
  --  Correo el día 28/01/2010.....
  ...
  SELECT NVL(t.flag_ingreso_boleta, 'S')
   ...
   select NVL(SUM(q.rem_externa),0), NVL(SUM(q.rem_imprecisa),0), 
          NVL(SUM(q.rem_retencion),0), SUM(NVL(q.sueldo, 0))
