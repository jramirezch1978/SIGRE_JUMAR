create or replace procedure usp_cal_telecredito2
 ( as_nropla        in string ,
...
Cursor c_trabaj is
  select  distinct a.cod_trabajador, a.imp_adelanto, m.nro_cnta_ahorro
  from adelanto_quincena a, maestro m
  where m.flag_cal_plnlla = '1' and
        m.flag_estado = '1' and
        m.nro_cnta_ahorro <> ' ' and
        m.cod_trabajador = a.cod_trabajador and
        (a.concep = '2310' and a.imp_adelanto <> 0) and
...
Select sum(a.imp_adelanto)
 into ln_neto_emp
 from adelanto_quincena a , maestro maes
where a.concep = '2310' and
      a.imp_adelanto <> 0 and
      (a.cod_trabajador = maes.cod_trabajador and maes.nro_cnta_ahorro <> ' ') and
...
Select count(*)
  ...
  from adelanto_quincena aq, maestro ma
  where aq.concep = '2310' and
        aq.imp_adelanto <> 0 and
        (aq.cod_trabajador = ma.cod_trabajador and ma.nro_cnta_ahorro <> ' ') and
