create or replace procedure usp_pla_cal_lic_sindic
...
ls_tipo_ina        char(3);
lk_gan_fij         constant char(3) := '097';
ls_concep_lic      concepto.concep%type;
...
--  Obtiene concepto de licencia sindical
select rpn.concep
  into ls_concep_lic
  from rrhh_nivel rpn
  where rpn.cod_nivel = lk_gan_fij;

--  Determina dias de licencia sindical
ln_diatra := 0 ;
...
--  Calcula licencia sindical si tiene dias
If ln_diatra > 0 then
  ...
  For rc_c in c_concepto ( ls_concep_lic ) Loop
