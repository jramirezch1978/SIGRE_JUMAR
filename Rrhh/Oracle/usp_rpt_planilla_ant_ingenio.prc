create or replace procedure usp_rpt_planilla_ant_ingenio
...
ln_gan_37  number(11,2) ; ln_gan_38  number(11,2) ; ln_gan_39  number(11,2) ;
                          ln_des_01  number(11,2) ; ln_des_02  number(11,2) ;
...
cursor c_maestro is 
  Select m.cod_trabajador, m.fec_ingreso, m.fec_cese,
         m.dni, m.nro_ipss, m.cod_cargo, m.cod_afp,
         m.nro_afp_trabaj, m.cod_seccion
  from maestro m
...
delete from tt_planilla_ingenio ;
...
For rc_mae in c_maestro Loop
  ...
  select count(*)
    ...
    where h.cod_trabajador = rc_mae.cod_trabajador and
          to_char(h.fec_calc_plan,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY') ;
  ...
  ln_gan_37 := 0 ; ln_gan_38 := 0 ; ln_gan_39 := 0 ;
  ...
  --  Halla descripcion de la seccion
  ls_desc_seccion := ' ' ;
  Select sec.desc_seccion
    into ls_desc_seccion
    from seccion sec
    where sec.cod_seccion = ls_seccion ;
  ...
  Select count(*)
    ...
    where hi.cod_trabajador = ls_codigo and
          hi.concep = '1413' and
          ad_fec_proceso between hi.fec_desde and hi.fec_hasta ;
  ln_contador := nvl(ln_contador,0) ;
  If ln_contador = 1 then
    Select hi.fec_desde, hi.fec_hasta
  ...
  Select count(*)
    ...
    where hc.cod_trabajador = ls_codigo and
          hc.concep = '1001' and
          to_char(hc.fec_calc_plan,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY') ;
  ln_contador := nvl(ln_contador,0) ;
  If ln_contador > 0 then
    Select hc.dias_trabaj, hc.horas_trabaj
      into ln_var_01, ln_var_02
      from historico_calculo hc
      where hc.cod_trabajador = ls_codigo and
            hc.concep = '1001' and
            to_char(hc.fec_calc_plan,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY') ;
    ln_var_01 := nvl(ln_var_01,0) ;
    ln_var_02 := nvl(ln_var_02,0) ;
  End if ;
  ...
  Select count(*)
    ...
    from historico_calculo hc
    where hc.cod_trabajador = ls_codigo and
          substr(hc.concep,1,2) = '11' and
          to_char(hc.fec_calc_plan,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY') ;
  ln_contador := nvl(ln_contador,0) ;
  If ln_contador > 0 then
    Select sum(hc.horas_trabaj)
      into ln_var_03
      from historico_calculo hc
      where hc.cod_trabajador = ls_codigo and
            substr(hc.concep,1,2) = '11' and
  ...
  --  Ganancias
  For ln_codniv in 501 .. 539 Loop

    ls_codniv := to_char(ln_codniv) ;
    Select sum(hc.imp_soles)
      into ln_importe
      from historico_calculo hc, rrhh_nivel_detalle rrhh
      where hc.cod_trabajador = ls_codigo and 
            to_char(hc.fec_calc_plan,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY') and
            rrhh.cod_nivel = ls_codniv and 
            rrhh.concep = hc.concep;
  ...
  Insert into tt_planilla_ingenio (
    ...
    gan_33, gan_34, gan_35, gan_36, gan_37, gan_38, gan_39,
  ...
  Values (
    ad_fec_proceso, ls_seccion, ls_desc_seccion, ls_codigo,
    ...
    ln_gan_33, ln_gan_34, ln_gan_35, ln_gan_36, ln_gan_37, ln_gan_38, ln_gan_39,
    ln_des_01, ln_des_02, ln_des_03, ln_des_04,
...
End usp_rpt_planilla_ant_ingenio ;
/
