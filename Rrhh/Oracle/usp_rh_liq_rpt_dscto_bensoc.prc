create or replace procedure usp_rh_liq_rpt_dscto_bensoc (
...
--  Lectura de movimiento de descuentos de beneficios sociales
cursor c_detalle is
...
--  ********************************************************************
--  ***   TEMPORAL PARA GENERAR DESCUENTOS POR BENEFICIOS SOCIALES   ***
--  ********************************************************************

delete from tt_liq_rpt_dsctos_bensoc ;

select p.grp_dscto_cta_cte into ls_grupo from rh_liqparam p
  where p.reckey = '1' ;
...
for rc_grp in c_grupos loop
  ...
  for rc_det in c_detalle loop
    ...
    insert into tt_liq_rpt_dsctos_bensoc (
...
end usp_rh_liq_rpt_dscto_bensoc ;
/
