create or replace procedure usp_pla_cal_enfermedad
...
ls_tipo_ina        char(3);
lk_gan_fij         constant char(3) := '095';
ls_concep_enf      concepto.concep%type;
...
--  Determina dias de enfermedad patronal ( 20 primeros dias )
ln_diatra := 0 ;
...
--  Calcula enfermedad patronal si tiene dias
If ln_diatra > 0 then
  ...
  For rc_c in c_concepto ( ls_concep_enf ) Loop
