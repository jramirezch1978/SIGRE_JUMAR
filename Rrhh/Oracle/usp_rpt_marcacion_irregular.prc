create or replace procedure usp_rpt_marcacion_irregular
...
For rc_con in c_consolidado_diario loop
  ...
  If ln_graba <> 0 then
    ...
    Select m.cod_trabajador, m.cod_area, m.cod_seccion, m.cencos
      into ls_cod_trabajador, ls_cod_area, ls_seccion, ls_cencos
      from maestro m
      where m.flag_estado = '1' and
            m.carnet_trabaj = ls_carnet and
            m.flag_marca_reloj = '1' ;
    ...
    Values (
      ...
      ls_nombres, ld_fecha_marcacion, ls_desc_dia, ls_turno_marcacion,
...
End usp_rpt_marcacion_irregular ;
/
