...
INSERT INTO tt_det_plla_calculada (cod_trabajador, concepto, cod_afp, imp_pagado, horas_trabaj, horas_pag, dias_trabaj)
select a.cod_trabajador, 
       a.concep, 
       a.cod_afp, 
       sum(a.imp_soles) as imp_soles, 
       sum(a.horas_trabaj) as horas_trabaj, 
       sum(a.horas_pag) as horas_pag, 
       sum(a.dias_trabaj) as dias_trabaj
       from 
(select m.cod_trabajador, c.concep, m.cod_afp, 
       sum(c.imp_soles) as imp_soles, 
       sum(c.horas_trabaj) as horas_trabaj, 
       sum(c.horas_pag) as horas_pag, 
       sum(c.dias_trabaj) as dias_trabaj 
...
group by m.cod_trabajador, c.concep, m.cod_afp 

UNION ALL 

select hc.cod_trabajador, hc.concep, hc.cod_afp, 
       sum(hc.imp_soles) as imp_soles, 
       sum(hc.horas_trabaj) as horas_trabaj, 
       sum(hc.horas_pagad) as horas_pagad, 
       sum(hc.dias_trabaj) as dias_trabaj 
  from historico_calculo hc, maestro m 
 where hc.cod_trabajador = m.cod_trabajador and 
       hc.cod_origen=as_origen and 
       hc.tipo_trabajador like as_tipo_trabaj and 
       trunc(hc.fec_calc_plan) between ad_fec_proceso_ini and ad_fec_proceso_fin         
group by hc.cod_trabajador, hc.concep, hc.cod_afp) a 

group by a.cod_trabajador, a.concep, a.cod_afp 
order by a.cod_trabajador, a.concep ;
