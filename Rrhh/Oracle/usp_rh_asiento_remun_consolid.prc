create or replace procedure usp_rh_asiento_remun_consolid (
...
cursor c_calculo is
  select c.concep, c.imp_soles, c.imp_dolar, c.tipo_doc_cc, c.nro_doc_cc
  from calculo c
...
--  Lectura del historico de calculo de la planilla por trabajador
...
select gc.concepto_gen
  ...
  where gc.grupo_calculo = ls_grupo_bas ;

select c.nro_horas
  into lk_nro_horas
  from concepto c
  where c.concep = ls_basico ;

select p.fec_inicio, p.fec_final into ld_fec_desde, ld_fec_hasta
  from rrhh_param_org p where p.origen = as_origen ;
...
/*  --  Determina numero provisional
  ...
  while rc_mae.cod_seccion = ls_seccion and c_maestro%found loop*/
    ...
    --  *************************************************************
    --  ***   Verifico si la data esta en la tabla CALCULO
    --  *************************************************************
    select count(*)
      into ln_verifica
      from calculo c
          ...
          if ln_distribucion = 0 then
            ...
            if ln_verifica > 0 then
              select nvl(c.cnta_cntbl_debe,' '), nvl(c.cnta_cntbl_haber,' ')
                ...
                from concepto_tip_trab_cnta c
                where c.concep = ls_concepto and c.tipo_trabajador = as_tipo_trabajador ;
            else
              raise_application_error( -20000, 'Concepto' || ' ' ||ls_concepto || ' ' || 'No
                                       tiene Cuentas Contables') ;
            end if ;
            ...
            if ls_cuenta_debe <> ' ' then
              if substr(ls_cuenta_debe,1,1) = '9' then
                ...
                if ln_contador > 0 then
                  ...
                  if ln_verifica = 0 then
                    raise_application_error( -20002, 'Cuenta' || ' ' ||ls_cuenta_debe || ' ' ||
                                             'No Existe por el Grupo Contable Para el Centro de Costo' || '  ' || ls_cencos || ' '||
                                             ls_concepto|| ' '|| as_tipo_trabajador) ;
                  end if ;
                end if ;
              end if ;
    ...
    --
              if ls_flag_cr <> '1' then
                 ...
                 ls_ind_codigo := ls_codigo ;
                 ls_tipo_doc   := rc_cal.tipo_doc_cc ;
                 ls_nro_doc    := rc_cal.nro_doc_cc ;
              end if ;
              ...
              /*
              Modificacion de Procedimiento: 04/10/2005
              Responsable: Jose Manuel Pomalaza Fernandez
              
              La insercion del NRO DE REFERENCIA 2 es para identificar
              Los Asientos que tienen distribucion o no.  Para evitar
              hacer selects unidos y prolongar el tiempo de procesamiento.
              */
              
              Update cntbl_pre_asiento_det c
                 Set c.imp_movsol = c.imp_movsol + ln_imp_soles,
                     c.imp_movdol = c.imp_movdol + ln_imp_dolar
               Where c.origen          = as_origen
                 And c.nro_libro       = ln_nro_libro
                 And c.nro_provisional = ln_provisional
                 And c.cnta_ctbl       = ls_cuenta_debe
                 And c.fec_cntbl       = ad_fec_proceso
                 And c.flag_debhab     = ls_flag_dh
                 And nvl(c.cencos,' ') = nvl(ls_ind_cencos,' ')
                 And nvl(c.tipo_docref ,' ') = nvl(ls_tipo_doc,' ')
                 And nvl(c.nro_docref1,' ')  = nvl(ls_nro_doc,' ')
                 And nvl(c.cod_relacion,' ') = nvl(ls_ind_codigo,' ')
                 And c.nro_docref2           = '1';
              ...
              If Sql%Notfound Then
                   ...
                   insert into cntbl_pre_asiento_det (
                     ...
                     tipo_docref, nro_docref1, nro_docref2, cencos, cod_relacion,
                   ...
                   values (
                     ...
                     ls_tipo_doc, ls_nro_doc, '1', ls_ind_cencos, ls_ind_codigo,
            ...
            if ls_cuenta_haber <> ' ' then
              if substr(ls_cuenta_haber,1,1) = '9' then
                ...
                if ln_contador > 0 then
                  ...
                  if ln_verifica = 0 then
                    raise_application_error( -20003, 'Cuenta' || ' ' ||ls_cuenta_haber || ' ' ||
                                             'No Existe por el Grupo Contable Para el Centro de Costo' || ' ' || ls_cencos) ;
                  end if ;
                end if ;
              end if ;
    ...
    --
              ...
              */
              ...
              Update cntbl_pre_asiento_det c
               ...
               Where c.origen          = as_origen
                 And c.nro_libro       = ln_nro_libro
                 And c.nro_provisional = ln_provisional
                 And c.cnta_ctbl       = ls_cuenta_haber
                 And c.fec_cntbl       = ad_fec_proceso
                 And c.flag_debhab     = ls_flag_dh
                 And nvl(c.cencos,' ') = nvl(ls_ind_cencos,' ')
                 And nvl(c.tipo_docref ,' ') = nvl(ls_tipo_doc,' ')
          ...
          if ln_distribucion = 1 then
            ...
            if ln_verifica > 0 then
              ...
              raise_application_error( -20001, 'Concepto' || ' ' ||ls_concepto || ' ' || 'No
                                       tiene Cuentas Contables') ;
            end if ;
            if ls_cuenta_debe <> ' ' then
              ls_flag_dh := 'D' ;
                ...
                while c_distribucion%found loop
                  ...
                  if ln_contador > 0 then
                      ...
                      select count(*)
                       ...
                       where c.cnta_ctbl = ls_cuenta_debe ;
                      if ln_verifica = 0 then
                        raise_application_error( -20004, 'Cuenta' || ' ' ||ls_cuenta_debe || ' '
                                                 || 'No Existe por el Grupo Contable Para el Centro de Costo' || ' ' || ls_cencos_dis) ;
                      end if ;
                  end if ;
    ...
    --
                    ...
                    if ls_flag_cp = '1' Then
                          ...
                          if ln_verifica > 0 then
                             select l.cnta_prsp
                              ...
                              raise_application_error( -20008, 'Labor' || ' ' ||ls_labor_dis || ' ' ||
                ...
                if ln_tot_dis_sol <> 0 then
                  ...
                  if ln_contador > 0 then
                    ...
                    ls_cuenta_debe := ls_grupo||substr(ls_cuenta_debe,3,8) ;
                    ln_verifica := 0 ;
                    select count(*) into ln_verifica from cntbl_cnta c
                      where c.cnta_ctbl = ls_cuenta_debe ;
                    if ln_verifica = 0 then
                      raise_application_error( -20005, 'Cuenta' || ' ' ||ls_cuenta_debe || ' '
                                               || 'No Existe por el Grupo Contable Para el Centro de Costo' || ' ' || ls_cencos) ;
                    end if ;
                  end if ;
                  select nvl(cta.flag_cencos,'0'), nvl(cta.flag_codrel,'0')
...
/*                   Update cntbl_pre_asiento_det c
                      Set c.imp_movsol = c.imp_movsol + ln_tot_dis_sol,
                          c.imp_movdol = c.imp_movdol + ln_tot_dis_dol
                    Where c.origen          = as_origen
                      And c.nro_libro       = ln_nro_libro
                      And c.nro_provisional = ln_provisional
                      And c.cnta_ctbl       = ls_cuenta_debe
    ...
    --
                ...
                while c_distribucion%found loop
                  ...
                  select count(*)
                    ...
                    where c.cencos = ls_cencos_dis ;
                  if ln_contador > 0 then
                    select nvl(c.flag_cta_presup,'0'), nvl(c.grp_cntbl,'0')
                      into ls_flag_cp, ls_grupo from centros_costo c
                      where c.cencos = ls_cencos_dis ;
                    ls_cuenta_haber := ls_grupo||substr(ls_cuenta_haber,3,8) ;
                    ln_verifica := 0 ;
                    select count(*) into ln_verifica from cntbl_cnta c
                      where c.cnta_ctbl = ls_cuenta_haber ;
                    if ln_verifica = 0 then
                      raise_application_error( -20006, 'Cuenta' || ' ' ||ls_cuenta_haber || ' '
                                               || 'No Existe por el Grupo Contable Para el Centro de Costo' || ' ' || ls_cencos_dis) ;
                    end if ;
                  end if ;
                  select nvl(cta.flag_cencos,'0'), nvl(cta.flag_codrel,'0')
                   ...
                   ln_item := ln_item + 1 ;
                    ...
                    if ls_flag_cp = '1' Then
                         ...
                         if ln_verifica > 0 then
                           ...
                           raise_application_error( -20009, 'Labor' || ' ' ||ls_labor_dis || ' ' ||
                                                    'No Tiene Cuenta Presupuestal') ;
                         end if ;
                ...
                if ln_tot_dis_sol <> 0 then
                  ...
                  if ln_contador > 0 then
                    ...
                    select count(*) into ln_verifica from cntbl_cnta c
                      where c.cnta_ctbl = ls_cuenta_haber ;
                    if ln_verifica = 0 then
                      raise_application_error( -20007, 'Cuenta' || ' ' ||ls_cuenta_haber || ' '
                                               || 'No Existe por el Grupo Contable Para el Centro de Costo' || ' ' || ls_cencos) ;
                    end if ;
                  end if ;
                  select nvl(cta.flag_cencos,'0'), nvl(cta.flag_codrel,'0')
...
/*                    End If;*/
    ...
    --
              if ls_flag_cr <> '1' then
                 ...
                 ls_ind_codigo := ls_codigo ;
                 ls_tipo_doc   := rc_hist.tipo_doc_cc ;
                 ls_nro_doc    := rc_hist.nro_doc_cc ;
              end if ;
...
end usp_rh_asiento_remun_consolid ;
/
