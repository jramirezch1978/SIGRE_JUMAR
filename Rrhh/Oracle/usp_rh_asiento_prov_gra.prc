CREATE OR REPLACE PROCEDURE usp_rh_asiento_prov_gra (
...
ls_cencos_destino    centros_costo.cencos%type ;
ls_flag_pre_cnta     cntbl_pre_asiento_det.flag_gen_aut%type;
...
--  Personal activo para generacion de asientos de gratificaciones
CURSOR c_maestro(an_ano in cntbl_asiento.ano%type, an_mes in cntbl_asiento.mes%type) is
  SELECT m.cod_trabajador, m.tipo_trabajador, m.cencos, m.centro_benef, r.importe 
    FROM maestro m, rh_prov_vacac_gratif_cts r 
...
values (
  ...
  ln_tipo_cambio, 'PROVISION DE GRATIFICACIONES PERIODO '||to_char(ln_ano)||'-'||to_char(ln_mes), ad_fec_proceso, sysdate, 
...
FOR c_m IN c_maestro(ln_ano, ln_mes ) LOOP
    ...
    -- Buscando la cuenta contable, según centro de costo
   SELECT count(*) 
     INTO ln_count 
     FROM cnt_conf_provis_plla c 
    WHERE c.tipo_trabajador = c_m.tipo_trabajador 
      AND c.flag_tipo_provision = 'G' 
      AND c.cencos = c_m.cencos ;
   
   IF ln_count = 0 THEN
     RAISE_APPLICATION_ERROR(-20000, 'No existe configuración en cnt_conf_provis_plla de '
                                     ||chr(13)||'Trabajador          ' || c_m.cod_trabajador 
                                     ||chr(13)||'Centro costo        ' || c_m.cencos 
                                     ||chr(13)||'Tipo de trabajador  ' || c_m.tipo_trabajador ) ;
   ...
   lb_ok := usf_cnt_pre_asiento_det(
           ...
           ad_fec_proceso, 'Provisión de gratificaciones',
           ...
           0, ls_cnta_prsp, -->0=ln_imp_movaju
           ls_cencos_destino, ls_flag_pre_cnta, 
           c_m.centro_benef ) ;
