create or replace procedure usp_pla_cal_apo_seg_agrario
  (  as_codtra       in maestro.cod_trabajador%type,
     ad_fec_proceso  in rrhhparam.fec_proceso%type
  ) is
   
lk_seg_agrario            constant char(3) :='310';  
ln_imp_seg_agrario        number(13,2);
ln_imp_seg_agrariod       number(13,2);
ln_fact_pago              concepto.fact_pago%type;
ln_imp_seg_agrario_acum   number(13,2); 
...
cursor c_seg_agrario (as_codtra maestro.cod_trabajador%type) is 
...
Select rpn.concep
  ...
  from rrhh_nivel rpn
  where rpn.cod_nivel = lk_seg_agrario ;
...
ln_imp_seg_agrario_acum := 0 ;
For rc_agr in c_seg_agrario (as_codtra) Loop 
  ln_imp_seg_agrario_acum := ln_imp_seg_agrario_acum + rc_agr.imp_soles ;
End Loop;
ln_imp_seg_agrario_acum := nvl(ln_imp_seg_agrario_acum,0) ;
 
If ln_imp_seg_agrario_acum > 0 then
  ln_imp_seg_agrario  := ln_imp_seg_agrario_acum * ln_fact_pago ; 
  ln_imp_seg_agrariod := ln_imp_seg_agrario / ln_tipcam ; 
  ...
  Values ( as_codtra, 
    ...
    0 ,    0 ,  0 , 
    ln_imp_seg_agrario, ln_imp_seg_agrariod,  ' ', 
    ' ',     ' ', 
    ' ',     ' ', 
    ' ',     ' ', 
    ' ',     ' ', 
...
End usp_pla_cal_apo_seg_agrario ;
/
