create or replace procedure usp_rh_ganfij_origen(
...
begin
   ...
   insert into tt_rh_ganfij_todo -- insertar todos las ganancais fijas
      ...
      select gdf.cod_trabajador, gdf.concep, gdf.imp_gan_desc 
         from gan_desct_fijo gdf 
            inner join maestro m on gdf.cod_trabajador = m.cod_trabajador
   ...
   insert into tt_rh_ganfij_todo -- insertar todos las ganancais fijas
      ...
      select gdf.cod_trabajador, gdf.concep, gdf.imp_gan_desc 
         from gan_desct_fijo gdf 
            inner join maestro m on gdf.cod_trabajador = m.cod_trabajador
         where substr(gdf.concep,1,2) = (select rp.grc_dsc_fijo from rrhhparam rp where rp.reckey = '1')
            and m.cod_origen = asi_cod_origen
            and m.tipo_trabajador = asi_tipo_trabajador;
   ...
   insert into tt_rh_ganfij_todo 
      (cod_trabajador, concep, importe)
      select trgt.cod_trabajador, '1099', sum(trgt.importe) 
         from tt_rh_ganfij_todo trgt 
            inner join maestro m on trgt.cod_trabajador = m.cod_trabajador
         where substr(trgt.concep,1,2) = (select rp.grc_gnn_fija from rrhhparam rp where rp.reckey = '1') 
            and m.cod_origen = asi_cod_origen
            and m.tipo_trabajador = asi_tipo_trabajador
         group by trgt.cod_trabajador;


   insert into tt_rh_ganfij_todo 
      (cod_trabajador, concep, importe)
      select trgt.cod_trabajador, '2299', sum(trgt.importe) 
         from tt_rh_ganfij_todo trgt
            inner join maestro m on trgt.cod_trabajador = m.cod_trabajador
         where substr(trgt.concep,1,2) = (select rp.grc_dsc_fijo from rrhhparam rp where rp.reckey = '1') 
            and m.cod_origen = asi_cod_origen
            and m.tipo_trabajador = asi_tipo_trabajador
         group by trgt.cod_trabajador;

end usp_rh_ganfij_origen;
/
