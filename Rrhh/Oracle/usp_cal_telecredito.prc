...
lk_neto             constant char(4) := '2354' ;   --  Concepto del neto a pagar
...
Select sum(c.imp_soles)
 into ln_neto_emp
 from calculo c, maestro maes
where c.concep = lk_neto and
      c.imp_soles <> 0 and
      (c.cod_trabajador = maes.cod_trabajador and maes.nro_cnta_ahorro <> ' ') and
...
Select count(*)
  ...
  from calculo ca, maestro ma
  where ca.concep = '2354' and
        ca.imp_soles <> 0 and
        (ca.cod_trabajador = ma.cod_trabajador and ma.nro_cnta_ahorro <> ' ') and
...
--  Dma y mes de proceso segun fecha
