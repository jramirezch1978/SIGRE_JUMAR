create or replace procedure usp_rh_boletas_pago (
  asi_fecha_proceso in string, aso_fecha_actual out string ) is
...
select p.cnc_total_ing into ls_concep_ingreso from rrhhparam p
  where p.reckey = '1' ;
     
select gc.concepto_gen into ls_concep_rem_bas from grupo_calculo gc
  where gc.grupo_calculo = ( select rcc.remunerac_basica from rrhhparam_cconcep rcc
                             where rcc.reckey = '1') ;

insert into tt_rh_boleta_cabecera ( 
  trabajador, basico, fecha, mes, semana, nombre, nacionalidad, cod_area,
  cod_seccion, fec_nacimiento, flag_sexo, direccion, dni, desc_cargo,
  fecha_ingreso, fecha_cese, nro_ips, nro_afp, cencos, cod_categ_sal,
  desc_afp, empresa_nombre, empresa_dir_calle, cod_origen, nombre_origen,
  tipo_trabajador, desc_tipo_trabajador, cod_afp )

select clc.cod_trabajador, gf.imp_gan_desc, clc.fec_proceso, to_number(to_char(clc.fec_proceso, 'mm')), to_number(rpad(to_char(c.ano_calc),4,'0') || lpad(to_char(c.semana_calc),2,'0')), cr.nombre, p.nacionalidad, m.cod_area,
  m.cod_seccion, m.fec_nacimiento, m.flag_sexo, m.direccion, m.dni, crg.desc_cargo,
  m.fec_ingreso, m.fec_cese, m.nro_ipss, m.nro_afp_trabaj, m.cencos, m.cod_categ_sal,
  afp.desc_afp, emp.nombre, trim(emp.dir_calle)||' '||trim(emp.dir_distrito), m.cod_origen, o.nombre,
  m.tipo_trabajador, tt.desc_tipo_tra, m.cod_afp
  from calculo clc
  ...
  where clc.fec_proceso = ld_calc_act and clc.concep = ls_concep_ingreso

  union all

select hcl.cod_trabajador, gf.imp_gan_desc, hcl.fec_calc_plan, to_number(to_char(hcl.fec_calc_plan, 'mm')), to_number(rpad(to_char(c.ano_calc),4,'0') || lpad(to_char(c.semana_calc),2,'0')), cr.nombre, p.nacionalidad, m.cod_area,
  m.cod_seccion, m.fec_nacimiento, m.flag_sexo, m.direccion, m.dni, crg.desc_cargo,
  m.fec_ingreso, m.fec_cese, m.nro_ipss, m.nro_afp_trabaj, m.cencos, m.cod_categ_sal,
  afp.desc_afp, emp.nombre, trim(emp.dir_calle)||' '||trim(emp.dir_distrito), m.cod_origen, o.nombre,
  m.tipo_trabajador, tt.desc_tipo_tra, m.cod_afp
  ...
  where hcl.fec_calc_plan = ld_calc_act and hcl.concep = ls_concep_ingreso and
        clc.concep is null ;
...
insert into tt_rh_boleta_concepto (
  trabajador, concepto, fecha, importe, horas, dias, desc_concepto )
...
select clc.cod_trabajador, clc.concep, clc.fec_proceso, clc.imp_soles, clc.horas_trabaj, clc.dias_trabaj, cpto.desc_breve
  ...
  where trunc(clc.fec_proceso) = trunc(ld_calc_act) and nvl(clc.imp_soles,0) <> 0
...
select hcl.cod_trabajador, hcl.concep, hcl.fec_calc_plan, hcl.imp_soles, hcl.horas_trabaj, hcl.dias_trabaj, cpto.desc_breve
  from historico_calculo hcl
         ...
         left outer join calculo clc on hcl.cod_trabajador = clc.cod_trabajador
            and hcl.concep = clc.concep
            and trunc(hcl.fec_calc_plan) = trunc(clc.fec_proceso)
  where trunc(hcl.fec_calc_plan) = trunc(ld_calc_act) and clc.concep is null and
        nvl(hcl.imp_soles,0) <> 0 ;
