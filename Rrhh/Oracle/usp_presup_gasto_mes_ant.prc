create or replace procedure usp_presup_gasto_mes_ant
  ( ad_fec_proceso in date, ad_mensaje in out char ) is
...
cursor c_calculo is
  ...
  from historico_calculo cal
  where cal.concep <> '1450' and cal.concep <> '2351' and
        cal.concep <> '2354' and cal.concep <> '3050' and
        cal.fec_calc_plan = ad_fec_proceso
...
for rc_cal in c_calculo loop
  ...
  if ls_cnta_prsp <> ' ' and ln_importe <> 0 then
    insert into tt_presupuesto_gasto_mes (
      ano, cencos, cnta_prsp, fecha, importe )
    values (
      ln_ano, ls_cencos, ls_cnta_prsp, ld_fecha, ln_importe ) ;
  end if ;
