create or replace procedure usp_rh_pago_alimentista (
...
cursor c_personal is
  select j.cod_trabajador, j.nom_pension, j.importe, j.nro_cnta_ahorro
  from judicial j, maestro m
  where j.cod_trabajador = m.cod_trabajador and m.flag_cal_plnlla = '1' and
        m.flag_estado = '1' and nvl(j.nro_cnta_ahorro,' ') <> ' ' and
...
select sum(nvl(j.importe,0)) into ln_neto_emp
  from judicial j, maestro m
  where j.cod_trabajador = m.cod_trabajador and m.flag_cal_plnlla = '1' and
        m.flag_estado = '1' and nvl(j.nro_cnta_ahorro,' ') <> ' ' and
        m.tipo_trabajador like as_tipo_trabaj and m.cod_origen = as_origen and
