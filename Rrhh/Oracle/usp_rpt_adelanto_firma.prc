create or replace procedure usp_rpt_adelanto_firma
 ( ln_con_convenio     in integer ,
   ln_sin_convenio     in integer  
 ) is
...
delete from tt_rpt_adelanto_firma ;
    ...
    If ls_convenio = '1' then
      ...
      If ln_registros > 0 then
        Select rtrim(ap.apel_paterno_apod)||' '||rtrim(ap.apel_materno_apod)||' '||
               nvl(rtrim(nombre1_apod),' ')||' '||nvl(rtrim(ap.nombre2_apod),' ')
      ...
      --  Insertar los Registro en la tabla tt_rpt_adelanto_firma
      If ln_adelanto_mes > 0 then
        Insert into tt_rpt_adelanto_firma
          (codigo, secuencia, nombres, nombres_apoderados,
           importe, fecha_proceso)
        Values
           (ls_codigo, ln_secuencia, ls_nombres, ls_nombres_apoderados,
           ln_adelanto_mes, ld_fecha_proceso ) ;
      End if ;
