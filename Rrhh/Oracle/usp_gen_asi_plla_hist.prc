create or replace procedure usp_gen_asi_plla_hist (
  as_tipo_trabajador in char, as_origen in char, as_usuario in char,
...
cursor c_calculo is
  ...
  where c.cod_trabajador = ls_codigo and c.imp_soles <> 0 and
        to_char(c.fec_calc_plan,'DD/MM/YYYY') = to_char(ad_fec_proceso,'DD/MM/YYYY') and
